...
NEW_COLUMNS_NAME = ["Lat", "Long", "Error",
...
# it's the new mandatory parameter
GOOGLE_API_KEY = "AIzaSyAQanzCC6g4sR3tgj8tmFlByqhGFVKBFZI"
    ...
    with open(INPUT_CSV_FILE, 'r') as csvinput:
        with open(OUTPUT_CSV_FILE, 'w') as csvoutput:
            ...
            header = [h.strip('"').strip()
                      for h in next(csvinput).split(DELIMITER)]
            ...
            for record in reader:
                ...
                location = geocode_address(
                    geo_locator, line_address, component_restrictions)
...
def geocode_address(geo_locator, line_address, component_restrictions=None, retry_counter=1):
    try:
        ...
        location = geo_locator.geocode(
            line_address, components=component_restrictions)
    ...
    # To catch generic geocoder errors.
        ...
        if hasattr(error, 'message'):
            ...
            error_message = error
        location_result = {"Lat": 0, "Long": 0, "Error": error_message,
                           "formatted_address": "", "location_type": ""}
    ...
    print("geocoded address : {0}".format(
        location_result["formatted_address"]))
    print("location type    : {0}".format(location_result["location_type"]))
    print(
        "Lat/Long         : [{0},{1}]".format(location_result["Lat"], location_result["Long"]))
    print("-------------------")
